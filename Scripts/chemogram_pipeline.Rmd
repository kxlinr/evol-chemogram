...
The signatures used here were extracted using the [CisSig pipeline](https://github.com/jessicascarborough/cissig) [published by Scarborough et al](https://www.nature.com/articles/s41698-023-00375-y.pdf). The parameters used for each signature were the same as CisSig, with the exception of the irinotecan, topotecan, and vorinostat signatures, which used a differential expression cutoff of 0.15 instead of 0.2.
...
```{r save_perms}
# acc_scores = as.data.frame(readRDS(here("perm10.rds"))) #ran the above code on the cluster
