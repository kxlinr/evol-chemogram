...
#testing the smaller length-optimized sigs
...
rm(norm_dose_resp, prep_dose_resp, raw_dose_resp) #cleanup
```
...
#   select(-score)%>%
#   mutate(cell_line = substr(cell_line, 6, nchar(cell_line)))
# 
# comp2 = scores_long %>%
...
#   labs(title = "Sig Scores From Different Expression Normalization Methods",
#        subtitle = "Norm to GAPDH vs Z-Score Across All Genes (Within Sample)",
...
surv_sup = left_join(surv_sup, fitted_dr, by=c("cell_line", "drug_abbrev")) %>%
  select(cell_line, subtype, drug_abbrev, score, rank, surv, LN_IC50, AUC)
```

```{r}
ggplot(surv_sup, aes(x=score, y=LN_IC50)) +
  geom_point(alpha=0.4, col = "#48705d") +
  labs(title="Predicted Sensitivity vs. IC50",
       x="Signature Score", 
       y="ln(IC50)")+
...
ggsave(here("Plots",
            paste0(length(sigs), "sig_score_v_ic50.png")),
       width=15, height=7)
```

```{r}
ggplot(surv_sup %>% filter(subtype %in% epi), aes(x=score, y=LN_IC50)) +
  geom_point(alpha=0.4, col = "#48705d") +
  labs(title="Predicted Sensitivity vs. IC50",
       subtitle = "in Epithelial Cancers",
       x="Signature Score", 
...
ggsave(here("Plots", "Epithelial Cancers",
            paste0(length(sigs), "sig_score_v_ic50_epi.png")),
       width=15, height=7)
```
...
correlation bw 10sig and 3 sig rankings performance w/i subtypes
